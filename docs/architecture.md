...
#### Credentials are rotatable in-place via `PATCH /admin/app-config`

`webhook_secret`, `pem`, and `client_secret` can all be rotated without re-creating the App or restarting gharp. The endpoint is documented in [docs/app-config-rotation.md](./app-config-rotation.md). Hot-swap works because every consumer reads the field fresh from the store on each request — `webhook_secret` on every signature check, `pem` on every JWT mint.

The endpoint is double-gated: `ADMIN_TOKEN` bearer auth (existing) plus the new `ALLOW_ADMIN_EDIT` kill-switch. With the flag off (default), the endpoint returns 403 even with a valid bearer. The same flag also gates `POST /jobs/{id}/retry` and `POST /jobs/{id}/cancel`. This is a deliberate "read-only by default" stance: a leaked admin token can't, on its own, damage state.
...
5. **Webhook secret / pem rotation UI.** **Shipped:** `PATCH /admin/app-config`
   plus a dashboard "Rotate credentials" panel. Double-gated by
   `ADMIN_TOKEN` and the new `ALLOW_ADMIN_EDIT` flag. See
   [docs/app-config-rotation.md](./app-config-rotation.md).
