...
| `ADMIN_TOKEN` | _(unset)_ | Optional bearer token for admin-capability APIs. Applies to jobs, stats, metrics, and rotation endpoints: `GET /jobs`, `GET /jobs/{job_id}`, `POST /jobs/{job_id}/retry`, `POST /jobs/{job_id}/cancel`, `GET /stats`, `GET /metrics`, `PATCH /admin/app-config`. If set, requests must send `Authorization: Bearer <token>`; if empty/unset, endpoints are open. **Mutating endpoints additionally require `ALLOW_ADMIN_EDIT=true`** — see below. |
| `ALLOW_ADMIN_EDIT` | `false` | Master kill-switch for mutating admin endpoints: `POST /jobs/{id}/retry`, `POST /jobs/{id}/cancel`, `PATCH /admin/app-config`. Auth ordering: the bearer is checked first — a missing or wrong `ADMIN_TOKEN` always returns `401 Unauthorized`, regardless of the flag (this matters for the dashboard's auth-prompt flow). With a valid bearer (or `ADMIN_TOKEN` unset) and the flag still `false`, mutating endpoints return `403 Forbidden`. The dashboard renders a banner and disables the corresponding controls. Set to `true` only when you need to perform a write — keeping this off by default means an accidentally-leaked admin token can't immediately damage state. See [app-config-rotation.md](./app-config-rotation.md). |
...
| `RUNNER_NAME_PREFIX` | `gharp-` | Operator-facing prefix for runner/container names. The reconciler scopes its orphan sweep to `RUNNER_NAME_PREFIX + <instance id> + "-"` (see `GHARP_INSTANCE_ID` below), so two gharp deployments sharing one docker daemon can keep the default prefix and still stay isolated — each instance gets its own auto-generated id. Empty string fails startup. |
| `GHARP_INSTANCE_ID` | _(auto-generated, persisted in the store)_ | A short identifier embedded between `RUNNER_NAME_PREFIX` and the per-runner suffix to scope container names. Auto-generated on first boot and persisted in the `instance` table so it survives restarts. Must match `^[a-zA-Z0-9][a-zA-Z0-9_.-]*$` (docker's container-name charset) when set via env; an invalid value fails startup. Override only if you need a stable, human-readable id (e.g. `prod`, `staging`); when set via env, the env value wins and is **not** written back to the store. **Footgun:** if you set this env var, run for a while, then unset it, the store-generated id takes over, the effective prefix changes, and the previous run's containers / GitHub-side registrations stop matching the orphan sweep — they leak until manually cleaned. Either keep the env var set permanently or never set it. |
| `RUNNER_LABELS` | `self-hosted` | Comma-separated list of capability labels this pool can satisfy. A `workflow_job` is accepted only if **every** one of its `runs-on` labels appears here, is covered by `RUNNER_DYNAMIC_LABEL_PREFIXES`, or is the implicit `self-hosted` label (matching GitHub's cumulative `runs-on` semantics — a runner must have all required labels to be eligible). Comparison is case-insensitive. To partition multiple gharp deployments by capability, use a unique non-`self-hosted` label per pool. |
| `RUNNER_DYNAMIC_LABEL_PREFIXES` | `gharp-` | Comma-separated list of label prefixes accepted without predeclaring every generated value in `RUNNER_LABELS`. Use these for per-job routing labels such as `gharp-build-${{ github.run_id }}-${{ github.run_attempt }}` so the spawned runner registers with a label unique to that job. These prefixes are not capability labels; keep hardware/OS/runtime requirements in `RUNNER_LABELS`. Set to comma-only input such as `,` to disable dynamic labels. |
