...
| `RUNNER_WORKDIR_ROOT` | _(unset)_ | Optional host path containing per-runner workdirs (`<root>/<container_name>/...`). When set, the reconciler performs automatic cleanup in two paths: immediate cleanup after runner teardown and a 5-minute periodic orphan sweep (with 5-minute age grace) for crash leftovers. Use this when `RUNNER_COMMAND` mounts host workdirs such as `/tmp/runner/{{.ContainerName}}/_work`. Leave unset to disable filesystem cleanup. |
