...
| `ALLOW_ADMIN_EDIT` | `false` | Master kill-switch for mutating admin endpoints: `POST /jobs/{id}/retry`, `POST /jobs/{id}/cancel`, `PATCH /admin/app-config`. When `false` (default), those endpoints return `403 Forbidden` regardless of whether a valid `ADMIN_TOKEN` was presented (401 is reserved for bad bearer when the flag is on). The dashboard renders a banner and disables the corresponding controls. Set to `true` only when you need to perform a write — keeping this off by default means an accidentally-leaked admin token can't immediately damage state. See [app-config-rotation.md](./app-config-rotation.md). |
