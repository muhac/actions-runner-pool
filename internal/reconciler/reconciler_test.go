...
// Legacy-name workdir cleanup: a runner row inserted by a pre-instance-id
// binary has a container name lacking the instance segment. The current
// containerNamePrefix is the compound form ("gharp-<instance>-"), so a
// strict HasPrefix check on that prefix would skip cleanup and leak the
// workdir. cleanupRunnerWorkdir scopes by the user prefix instead.
func TestReconcile_GhostRunner_CleansLegacyWorkdir(t *testing.T) {
	root := t.TempDir()
	// Name shape predates the instance-id segment — only "gharp-" + jobID.
	container := "gharp-1-legacy"
	if err := os.MkdirAll(filepath.Join(root, container, "_work"), 0o755); err != nil {
		t.Fatalf("mkdir workdir: %v", err)
	}
	st := &fakeStore{rows: []*store.Runner{{
	...
	dk := &fakeDocker{exists: map[string]bool{}}
	// containerNamePrefix is the compound form; userPrefix is the bare prefix.
	r := New(st, dk, nil, quietLog(), 1*time.Hour, "gharp-instX-", "gharp-", root, nil, 0)
	r.nowFn = func() time.Time { return time.Unix(1_700_000_000, 0) }
	r.Reconcile(context.Background())
	if _, err := os.Stat(filepath.Join(root, container)); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected legacy workdir removed, stat err=%v", err)
	}
}
